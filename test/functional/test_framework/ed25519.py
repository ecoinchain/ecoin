...
class PureEdDSA:
    ...
    #Expand key.
        ...
        privkey_left=self.__clamp(khash[:self.b//8])
        a=from_le(privkey_left)
        privkey_right=khash[self.b//8:]
        privkey=privkey_left+privkey_right
        #Return the key pair (public key is A=Enc(aB).
        return privkey,(self.B*a).encode()
    #Sign with key pair.
    def sign(self,privkey,pubkey,msg,ctx,hflag):
        privkey_left=privkey[:self.b//8]
        a=from_le(privkey_left)
        seed=privkey[self.b//8:]
