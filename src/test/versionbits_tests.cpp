...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    for (unsigned int i=1; i<mainnetParams.nMinerConfirmationWindow - 4; i++) {
    ...
    nTime = nStartTime;
    for (unsigned int i=mainnetParams.nMinerConfirmationWindow - 4; i<=mainnetParams.nMinerConfirmationWindow; i++) {
        lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
    }
    ...
    for (unsigned int i=0; i<mainnetParams.nMinerConfirmationWindow - 1; i++) {
        lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
        nHeight += 1;
    }
