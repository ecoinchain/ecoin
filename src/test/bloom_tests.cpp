...
{
    std::string strSecret = std::string("2HJq4GiaremictdqLogFeohMYKeaT8i1TXrT8LWMhNW8LmnCqk3MNnCVjv1eW1mTctn6uS81Lj9V3rXb3fPTsZ4gGMXE3fS");
    CKey key = DecodeKey(strSecret);
    ...
    std::vector<unsigned char> vch = ParseHex("03eb1a1e080000000000000001");
    std::vector<char> expected(vch.size());
...
// todo 这个测试做不了，hash函数改为了sha3之后，原bitcoin上真实的交易数据无法使用
...
// todo 这个测试做不了，因为13b8a这个区块是bitcoin主网络上的区块
